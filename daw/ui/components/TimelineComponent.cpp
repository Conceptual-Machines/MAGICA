...
void TimelineComponent::mouseDown(const juce::MouseEvent& event) {
    ...
    std::cout << "🎯 MOUSE DOWN REACHED TIMELINE COMPONENT! Y=" << event.y << " X=" << event.x
              << std::endl;
    std::cout << "🎯 Timeline bounds: width=" << getWidth() << " height=" << getHeight()
              << std::endl;
    ...
    int playheadZoneStart =
        getHeight() - 10;  // Bottom 10 pixels for numbers + ticks (even larger zoom area)
    ...
    std::cout << "Sections (0-" << sectionsHeight << "), Zoom (" << sectionsHeight << "-"
              << playheadZoneStart << "), Playhead (" << playheadZoneStart << "-" << getHeight()
              << ")" << std::endl;
    std::cout << "In Sections: " << (inSectionsArea ? "YES" : "NO")
              << ", In Zoom: " << (inZoomZone ? "YES" : "NO")
              << ", In Playhead: " << (inPlayheadZone ? "YES" : "NO") << std::endl;
    std::cout << "Numbers area: " << (getHeight() - 35) << "-" << (getHeight() - 10)
              << ", Ticks area: " << (getHeight() - 10) << "-" << (getHeight() - 2) << std::endl;

    // Zone 1: Playhead zone (bottom area with ticks and numbers)
    if (inPlayheadZone) {
        DBG("ENTERING PLAYHEAD ZONE LOGIC");
        std::cout << "🎯 *** PLAYHEAD ZONE TRIGGERED *** Y=" << event.y
                  << " playheadStart=" << playheadZoneStart << std::endl;
...
void TimelineComponent::mouseMove(const juce::MouseEvent& event) {
    ...
    if (std::abs(event.y - lastY) > 2) {  // Only log when Y changes significantly
        std::cout << "🎯 MOUSE MOVE: Y=" << event.y << " (max should be " << (getHeight() - 1)
                  << ")" << std::endl;
        lastY = event.y;
    }
}
...
void TimelineComponent::mouseDrag(const juce::MouseEvent& event) {
    ...
    if (!startedInPlayheadZone) {
        std::cout << "🎯 ZOOM DRAG: startedInPlayheadZone=false, deltaY="
                  << std::abs(event.y - zoomStartY) << std::endl;
        ...
        if (deltaY > 3) {
            ...
            const int maxDeltaY =
                800;  // Increased limit to allow more movement within component bounds
            ...
            double sensitivity;
            std::cout << "🎯 ZOOM CHECK: actualDeltaY=" << actualDeltaY
                      << " (clamped), zoomStartValue=" << zoomStartValue << std::endl;
            ...
            if (actualDeltaY > 0) {
                ...
                if (isShiftHeld) {
                    ...
                    std::cout << "🎯 ZOOM IN: sensitivity=" << sensitivity << " (SHIFT TURBO)"
                              << std::endl;
                } else if (isAltHeld) {
                    sensitivity = config.getZoomInSensitivity() * 0.6;  // Alt is 60% of normal
                    std::cout << "🎯 ZOOM IN: sensitivity=" << sensitivity << " (ALT FAST)"
                              << std::endl;
                } else {
                    sensitivity = config.getZoomInSensitivity();
                    std::cout << "🎯 ZOOM IN: sensitivity=" << sensitivity << " (SENSITIVE)"
                              << std::endl;
                }
            } else {
                ...
                if (isShiftHeld) {
                    ...
                    std::cout << "🎯 ZOOM OUT: sensitivity=" << sensitivity << " (SHIFT EXTREME)"
                              << std::endl;
                } else if (isAltHeld) {
                    sensitivity = config.getZoomOutSensitivity() * 0.75;  // Alt is 75% of normal
                    std::cout << "🎯 ZOOM OUT: sensitivity=" << sensitivity << " (ALT CONTROLLED)"
                              << std::endl;
                } else {
                    sensitivity = config.getZoomOutSensitivity();
                    std::cout << "🎯 ZOOM OUT: sensitivity=" << sensitivity
                              << " (STANDARD AGGRESSIVE)" << std::endl;
                }
            }
            ...
            if (linearZoomFactor < minZoomFactor) {
                std::cout << "🎯 ZOOM FACTOR CLAMPED TO MIN: " << linearZoomFactor << " -> "
                          << minZoomFactor << std::endl;
                linearZoomFactor = minZoomFactor;
            } else if (linearZoomFactor > maxZoomFactor) {
                std::cout << "🎯 ZOOM FACTOR CLAMPED TO MAX: " << linearZoomFactor << " -> "
                          << maxZoomFactor << std::endl;
            ...
            if (timelineLength > 0 && viewportWidth > 0) {
                ...
                minZoom =
                    juce::jmax(minZoom, config.getMinZoomLevel());  // But not below config minimum

                std::cout << "🎯 DYNAMIC MIN ZOOM: timelineLength=" << timelineLength
                          << ", viewportWidth=" << viewportWidth
                          << ", availableWidth=" << availableWidth << ", minZoom=" << minZoom
                          << std::endl;
            }
            ...
            if (std::isnan(newZoom) || newZoom < minZoom) {
                newZoom = minZoom;
                std::cout << "🎯 ZOOM CLAMPED TO MIN: " << newZoom << " (FULL TIMELINE VIEW)"
                          << std::endl;
            ...
            std::cout << "🎯 ZOOM: factor=" << scaledZoomFactor << ", newZoom=" << newZoom
                      << std::endl;
